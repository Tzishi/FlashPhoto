...
bool operator== (const PixelBuffer& buf1, const PixelBuffer& buf2) {
  if (buf1.width() != buf2.width() || buf1.height() != buf2.height()) {
    return false;
  } else {
    for (int i = 0; i < buf1.height(); i++) {
      for (int j = 0; j < buf1.width(); j++) {
        if (buf1.get_pixel(j, i) == buf2.get_pixel(j, i)) {
          continue;
        } else {
          return false;
        }
